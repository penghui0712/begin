...
$(function(){
  ...
  $("#activity_save").click(function(){
	  ...
	  var activityDays = $("#activity_days").val().trim();
	  ...
	  var activityPlace = $("input[id=rg_cb1]:checked").val();
	  if(activityPlace == undefined){
		  activityPlace = "出发地与培训地一致";
	  }
	  var phoneNum = $("#phoneNum").val().trim();
	  var multiPrice = $("input[id=rg_cb2]:checked").val();
	  if(multiPrice == undefined){
		  multiPrice = "单批次价格";
	  }
	  var categoryType = $("#category_type").val().trim();
	  var activityPrice = $("#activity_price").val().trim();
	  var count = $("#count").val().trim();
	  var stock = $("#stock").val().trim();
	  var balance = $("input[id='rg_cb3']:checked").val();
	  ...
	  var discountRule = JSON.stringify(discount);//将数组转化为json字符串
	  ...
	  var childGoods = JSON.stringify(child_goods);//将数组转化为json字符串
	  ...
	  var relatedProducts = JSON.stringify(related_products );//将数组转化为json字符串
