...
$(function(){
  ...
  function changeLayout(from,to){
     ...
     //更新成功回调函数
      ...
      $(".ss_item").each(function(index,element){
        var count=$(element).children().length;
        var height=(count%9==0?count/9:parseInt(count/9)+1)*50+1;
        $(".fq_item").eq(index).height(height);
        $(".sm_item").eq(index).height(height);
      });
